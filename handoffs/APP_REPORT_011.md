...
describes). Published instead via the GitHub web editor (browser session logged in as the
athlete), one file per commit, directly on a new `cr-011-tuesday-run-intervals` branch off
`main` — the same fallback the note describes for a no-credentials sandbox, extended here from
single-file amends to a full multi-file change. Local commit hashes (`34355c0`, `9ce1dd8`) exist
only in the sandbox's local clone; the GitHub-side commits on this branch are new hashes, visible
in the branch's own history/PR.

**Also recorded here**: an early attempt to publish this branch resulted in the local commits
landing directly on `main` with no PR (a push happened from outside this sandboxed session,
against a wrongly-configured upstream — `git checkout -b ... origin/main` had set the new
branch's tracking to `main`). This was caught and reverted on `main` (two revert commits, pushed
by the athlete from a real terminal) before this branch was (re)built via the web editor. Flagging
this so it's not a mystery later if `main`'s history shows the revert pair.
